...
TEST(Beat, BeatLowPlaysDuringWave) {
    ...
    ast::test::tickFrames(game, 60);  // 1 s — covers at least one beat at max interval (0.9 s)
}
...
TEST(Beat, BeatAlternatesLowHigh) {
    ...
    ast::test::tickFrames(game, 120);  // 2 s — enough for two beats at max interval
}
...
TEST(Beat, NoBeatWhenWaveClear) {
    ...
    ast::test::tickFrames(game, 60);  // 1 s — well within the 2 s inter-wave delay
}
