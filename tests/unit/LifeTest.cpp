...
TEST(Life, NoRespawnBeforeDelay) {
    ...
    ast::test::startGame(game);
    killShip(game);
    ast::test::tickFrames(game, 60);  // 1 second — well before the 3-second respawn
    EXPECT_EQ(game.state(), ast::GameState::PlayerDead);
}
...
TEST(Life, ShipRespawnsAfterDelay) {
    ...
    ast::test::startGame(game);
    killShip(game);
    ast::test::tickFrames(game, 185);  // > 3 seconds — respawn guaranteed
...
TEST(Life, InvincShipSurvivesCollision) {
    ...
    ast::test::tickFrames(game, 185);         // respawned at centre with invincTimer > 0
    ast::test::tickFrames(game, 1);           // kill-rock still at centre; invincTimer still > 0
    EXPECT_TRUE(game.ship().active);
