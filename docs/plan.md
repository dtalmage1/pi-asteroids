...
| Backlog progress | **INF-1** ✓ Done. **INF-2** ✓ Done. **INF-3** ✓ Done. **INF-4** ✓ Done. **INF-5** ✓ Done — deploy + integration test scripts; RPi ctest verified. Slice 1 complete. **CORE-1** ✓ Done — foundation types, IRenderer, Sdl2Renderer stub, MockRenderer; 9/9 tests on both targets. **CORE-2** ✓ Done — IInputSource, InputState, Sdl2InputSource, MockInputSource; 12/12 tests on both targets. **CORE-3** ✓ Done — IAudioSink, NullAudioSink, Sdl2AudioSink stub, MockAudioSink; 16/16 tests on both targets. **CORE-4** ✓ Done — Platform RAII, 60 Hz game loop, blank window on RPi; 16/16 tests unaffected on both targets. **CORE-5** ✓ Done — Game class, GameState enum, state machine scaffold; 19/19 tests on both targets. **ENT-1** ✓ Done — physics helpers, collision, Ship struct, ship drift; 37/37 tests on both targets. **RND-1** ✓ Done — drawLine, drawLineStrip implemented; 37/37 tests on both targets. **RND-2** ✓ Done — ship chevron rendered each frame, spawns at screen centre; 38/38 tests on both targets. **ENT-2** in progress. |
