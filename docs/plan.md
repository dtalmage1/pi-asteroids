...
| Backlog progress | **INF-1** ✓ Done. **INF-2** ✓ Done. **INF-3** ✓ Done. **INF-4** ✓ Done. **INF-5** ✓ Done — deploy + integration test scripts; RPi ctest verified. Slice 1 complete. **CORE-1** ✓ Done — foundation types, IRenderer, Sdl2Renderer stub, MockRenderer; 9/9 tests on both targets. **CORE-2** ✓ Done — IInputSource, InputState, Sdl2InputSource, MockInputSource; 12/12 tests on both targets. **CORE-3** ✓ Done — IAudioSink, NullAudioSink, Sdl2AudioSink stub, MockAudioSink; 16/16 tests on both targets. **CORE-4** ✓ Done — Platform RAII, 60 Hz game loop, blank window on RPi; 16/16 tests unaffected on both targets. **CORE-5** ✓ Done — Game class, GameState enum, state machine scaffold; 19/19 tests on both targets. **ENT-1** ✓ Done — physics helpers, collision, Ship struct, ship drift; 37/37 tests on both targets. **RND-1** ✓ Done — drawLine, drawLineStrip implemented; 37/37 tests on both targets. **RND-2** ✓ Done — ship chevron rendered each frame, spawns at screen centre; 38/38 tests on both targets. **ENT-2** ✓ Done — ship controls, Attract→Playing on START; 43/43 tests on both targets. **ENT-3** ✓ Done — Asteroid struct, shape gen, movement + wrap; 54/54 tests on both targets. **RND-3** ✓ Done — asteroid wireframe rendering; 57/57 tests on both targets. **ENT-4** ✓ Done — Projectile struct, firing, lifetime, 4-shot cap; 65/65 tests on both targets. **RND-4** ✓ Done — projectile line-segment rendering; 67/67 tests on both targets. **ENT-5** ✓ Done — player projectile vs asteroid circle-circle collision; 69/69 tests on both targets. **ENT-6** ✓ Done — asteroid splitting (Large→2 Medium, Medium→2 Small, Small→nothing); 73/73 tests on both targets. **ENT-7** ✓ Done — ship–asteroid collision, PlayerDead transition; 77/77 tests on both targets. **GAME-1** ✓ Done — scoring (20/50/100 pts), score() accessor, 5 new tests; 82/82 tests on both targets. **GAME-2** ✓ Done — wave management, edge spawning, 2s inter-wave delay, waveNumber() accessor; 9 new tests; 91/91 tests on both targets. **GAME-3** ✓ Done — lives system (3 lives, respawn after 3s, 3s invincibility with flash, extra life at 10k pts, GameOver state); lives(), tickRespawn(); 10 new tests; 101/101 tests on both targets. **GAME-4** ✓ Done — GameOver state: 5s auto-transition to Attract or immediate on START; "GAME OVER" + score overlay via new stroke font (Glyph.hpp/cpp); score and lives reset on new game; 12 new tests; 113/113 tests on both targets. **RND-5** ✓ Done — HUD: score top-left via drawString, lives icons top-right via scaled buildShipVertices; shown only in Playing/PlayerDead; 5 new tests; 118/118 tests on both targets. **GAME-5** ✓ Done — Attract screen: "ASTEROIDS" + "PRESS START" stroke-font overlay; Glyph.cpp extended with D/I/P/S/T; 5 new tests; 123/123 tests on both targets. **RND-6** ✓ Done — Explosion particles: 128-slot pool, 8 per asteroid / 12 per ship, fading alpha, position integrated + wrapped; 6 new tests; 129/129 tests on both targets. **GAME-6** ✓ Done — ScoreTable: top-5 session scores sorted descending, submitted on GameOver→Attract (timer or START), displayed on Attract screen; 9 new tests; 138/138 tests on both targets. **GAME-7** ✓ Done — Hyperspace: random warp, 1-in-5 destruction chance via deterministic LCG seed; 8 new tests; 146/146 tests on both targets. **SAU-1** ✓ Done / **SAU-4** ✓ Done — Saucer entity + wireframe: 15s spawn timer (counts in Playing + PlayerDead), traverses screen at 80 px/s, exits and respawns; two-part polygon (6-pt hexagon body + 4-pt dome strip); saucer timer fix (was frozen in PlayerDead); 9 new tests; 155/155 tests on host. **SAU-2** ✓ Done — large saucer random-direction fire; player projectile vs saucer collision; 200 pts; saucer fire does not destroy asteroids; 4 new tests. **SAU-3** ✓ Done — small saucer aimed fire with wave-scaled spread; ship vs saucer projectile and body collision; 1000 pts; invincibility respected; 5 new tests. Slice 6 complete. **POL-1** ✓ Done — SDL_JOYDEVICEADDED/REMOVED handling in Sdl2InputSource; joystick unique_ptr reconnects on add, nulls on remove; ship coasts naturally (all-false inputs) while disconnected; 3 new tests. **POL-2** ✓ Done — SDL_Init split: VIDEO+JOYSTICK fatal, AUDIO non-fatal (logs warning, continues); Sdl2AudioSink initialised_ flag makes audio failure silent at runtime. **POL-3** ✓ Done — --width/--height/--fullscreen CLI flags in main.cpp; all entity sizes and physics scale via scale_=screenSize.y/600; 2 new tests. **POL-4** ✓ Done — FPS logged to stderr every 2 s in debug builds (#ifndef NDEBUG); vsync via ASTEROIDS_VSYNC compile flag. All 42 backlog items complete. Pre-release checklist in progress. |
| `docs/architecture.md` | **M2 final** — all open questions resolved |
| `docs/requirements.md` | **M1 outline complete** — scope decisions confirmed, parameters TBD at feature selection; IR-8 hardened for CORE-3 |
